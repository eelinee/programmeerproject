...
This script creates the scatterplot visualisation and contains the functions to 
update the scatterplot to show data of a different year.
*/
...
function createScatter(data1, data2, data3) {
	...
	scatterWidth = width * 0.56 - margin.left - margin.right 
		- legendMarginScatter;
	...
	scatterSvg = d3.select(".scatterDiv")
		...
		.attr("width", scatterWidth + margin.left + margin.right 
			+ legendMarginScatter)
		.attr("height", scatterHeight + margin.bottom + margin.top 
			+ bottomMarginScatter)
	...
	var lableText = ["Deaths per 100 000 population", 
...
function createCheckBoxes(xVariables, yVariables, scatterSvg) {
	...
	scatterSvg.selectAll(".yCheckBox")
		...
		.attr("y", function(d, i) {return 270 + 55 * i})
		.attr("width", 150)
		.attr("height", 50)
	...
	scatterSvg.selectAll("#yCheckText1")
		...
		.attr("id", "yCheckText1")
		.attr("x", scatterWidth + 125)
		.attr("y", function(d, i) {return 293 + 55 * i})
		.text(function(d) { 
			if(d == "With handwashing facilities at home") {
				return wrapText(d, 2)[0];
			}
			else {
				return wrapText(d, 3)[0];
			}
		});
	...
	scatterSvg.selectAll("#yCheckText2")
		...
		.attr("id", "yCheckText2")
		.attr("x", scatterWidth + 125)
		.attr("y", function(d, i) {return 310 + 55 * i})
		.text(function(d) {
			if(d == "With handwashing facilities at home") {
				return wrapText(d, 2)[1];
			}
			else {
				return wrapText(d, 3)[1];
			}
		});
	...
	scatterSvg.selectAll(".xCheckText")
		...
		.attr("y", function(d, i) {
			return scatterHeight + 68 + 35 * Math.floor(i / 2);
		})
